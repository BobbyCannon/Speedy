	...
	where T : WindowsServiceOptions
{
	...
	public static void InstallService(T options)
	{
		WindowsServiceInstaller.InstallService(options.ServiceFilePath, options.ToServiceString(), options.ServiceName, options.ServiceDisplayName, ServiceStartMode.Automatic);

		// Create the source, if it does not already exist.
		if (!EventLog.SourceExists(options.ServiceName))
		{
			...
			EventLog.CreateEventSource(options.ServiceName, "Application");
		}
	}
	...
	/// </summary>
	/// <param name="options"> The service options. </param>
	public static void UninstallService(T options)
	{
		WindowsServiceInstaller.UninstallService(options.ServiceName);

		// If the source exists we want to remove it
		if (EventLog.SourceExists(options.ServiceName))
		{
			EventLog.DeleteEventSource(options.ServiceName);
		}
	}
