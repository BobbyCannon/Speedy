...
namespace Speedy.UnitTests.Runtime;
...
public class RuntimeInformationShouldCache
{
    ...
    public void GetRuntimeInformation()
    {
		...
		var scenarios = new Test[]
		{
			...
			new MockSetup<RuntimeInformation, string>(mock, "ApplicationFileName", "FileName.exe"),
			new MockSetup<RuntimeInformation, string>(mock, "ApplicationFilePath", "C:\\Test\\FileName.exe"),
			...
			new MockSetup<RuntimeInformation, Version>(mock, "DotNetRuntimeVersion", new Version(5, 4, 3, 2))
		};

        var allProperties = typeof(RuntimeInformation)
            .GetCachedProperties()
            .Select(x => x.Name)
			.Except(["Count", "IsShuttingDown", "Item", "Keys", "Values"])
            .ToList();
