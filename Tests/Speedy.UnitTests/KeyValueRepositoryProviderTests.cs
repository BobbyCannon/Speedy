...
[TestClass]
public class KeyValueRepositoryProviderTests : SpeedyUnitTest
{
	#region Methods
	...
	public void ArchiveRepositoryInvalidName()
	{
		Cleanup();
		var provider = new KeyValueRepositoryProvider(TestHelper.Directory);
		ExpectedException<FileNotFoundException>(() => provider.ArchiveRepository("Repository1"), "The file could not be found.");
	}
	...
	public void UnarchiveRepositoryInvalidName()
	{
		Cleanup();
		var provider = new KeyValueRepositoryProvider(TestHelper.Directory);
		ExpectedException<FileNotFoundException>(() => provider.UnarchiveRepository("Repository1"), "The file could not be found.");
	}
	...
	public void WriteAllTypes()
	{
		...
		{
			{ "Folder", @"C:\Users\bobby\Desktop" },
			{ "Integer", 1 },
			{ "Decimal", 1.23456 },
			{ "TimeSpan", TimeSpan.FromSeconds(654321) }
		};

		var repository = (KeyValueRepository<string>) KeyValueRepository.Create(TestHelper.Directory, "Settings");

		foreach (var scenario in scenarios)
		{
			repository.Write(scenario.Key, scenario.Value.ToString());
		}

		repository.Save();

		File.ReadAllText(repository.FileInfo.FullName).Dump();

		repository = (KeyValueRepository<string>) KeyValueRepository.Create(TestHelper.Directory, "Settings");

		foreach (var scenario in scenarios)
		{
			var actual = repository.Read(scenario.Key);
			AreEqual(scenario.Value.ToString(), actual);
		}
	}
