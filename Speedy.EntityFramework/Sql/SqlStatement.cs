...
namespace Speedy.EntityFramework.Sql
{
	...
	public class SqlStatement
	{
		...
		internal SqlStatement(SqlTableInformation tableInformation)
		{
			...
			ParametersByColumnName = new Dictionary<string, IList<object>>();
			ParameterNameByColumnName = new Dictionary<string, IList<string>>();
		...
		/// </summary>
		public IDictionary<string, IList<string>> ParameterNameByColumnName { get; }
		...
		/// </summary>
		public IDictionary<string, IList<object>> ParametersByColumnName { get; }
		...
		{
			var parameterName = !ParameterNameByColumnName.TryGetValue(columnName, out var pNames)
			...
			switch (TableInformation.ProviderType)
			{
				case DatabaseProviderType.Sqlite:
				{
					...
					if (parameter == null)
					{
						...
						ParametersByColumnName.AddIfMissing(columnName, parameter);
						ParameterNameByColumnName.AddIfMissing(columnName, parameter.ParameterName);
					}
					else
					{
		...
		internal string GetSetColumnList(IDictionary<string, IList<string>> columnsAndParameters)
		{
			return string.Join(", ", columnsAndParameters.Select(x => $"{TableInformation.ProviderPrefix}{x.Key}{TableInformation.ProviderSuffix} = @{x.Value.First()}"));
		}

		internal IDictionary<string, IList<string>> GetSqlColumnParameterNames(bool includePrimaryKeys = false, bool onlyIncludePrimaryKeys = false, params string[] excludedColumns)
		{
			#if NETSTANDARD
			if (onlyIncludePrimaryKeys)
			{
				return TableInformation
					...
					.Select(x =>
					{
						...
						return (columnName, new List<string> { parameterName });
					})
					.ToDictionary(x => x.columnName, x => (IList<string>) x.Item2);
			}
			...
			if (onlyIncludePrimaryKeys)
			{
				return TableInformation
					...
					.Where(x =>
					{
						...
						return x.IsPrimaryKey() && !excludedColumns.Contains(columnName);
					})
					.Select(x =>
					{
						var dbType = GetSqlType(x.PropertyInfo.PropertyType);
			...
			TableInformation.Properties
				...
				.ForEach(x =>
				{
					var dbType = GetSqlType(x.PropertyInfo.PropertyType);
					var columnName = x.GetColumnName(StoreObjectIdentifier.Table(TableInformation.TableName, TableInformation.SchemaName));
					AddOrUpdateParameter(columnName, dbType);
				});
			#endif
		...
		internal string GetWhereColumnList(IDictionary<string, IList<string>> columnsAndParameters)
		{
			return string.Join(" AND ", columnsAndParameters.Select(x =>
			{
				var key = $"{TableInformation.ProviderPrefix}{x.Key}{TableInformation.ProviderSuffix}";
				return string.Join(" AND ", $"{key} = @{x.Value.First()}");
			}));
		}
