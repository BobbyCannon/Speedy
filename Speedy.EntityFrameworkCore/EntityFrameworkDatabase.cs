...
namespace Speedy.EntityFrameworkCore
{
	...
	public abstract class EntityFrameworkDatabase : DbContext, ISyncableDatabase
	{
		...
		{
			var connectionString =ConfigurationManager.ConnectionStrings[_nameOrConnectionString]?.ConnectionString;
			optionsBuilder.UseSqlServer(connectionString ?? _nameOrConnectionString);
		...
		{
			var entityType = typeof(IEntityTypeConfiguration);
			var assembly = Assembly.GetAssembly(GetType());
			var typesToRegister = assembly.GetTypes()
				.Where(p => entityType.IsAssignableFrom(p));
